...
def get_public_user_profile_view(user_id):
    try:
        ...
        return jsonify({"message": str(e), "success": False, "location": "view get public user profile repo"}), 500
...
def update_user_profile_view(user, user_request):
    try:
        ...
        return jsonify({"message": str(e), "success": False, "location": "view update user profile request validation"}), 400
    
    # not sure how to test
    except Exception as e:
        db.session.rollback()
        return jsonify({"message": str(e), "success": False, "location": "view update user profile repo"}), 500
